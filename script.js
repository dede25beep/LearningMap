...
function playVideo() {
  ...
  const iframe = document.getElementById('lm-iframe');
  const src = iframe.dataset.src;
  if (!src || src === 'SEU_VIDEO_EMBED_AQUI') {
    alert('Configure o link do vídeo no atributo data-src do iframe.');
    return;
  }
  iframe.src = src;
...
function toggleMobileMenu() {
  document.querySelector('.lm-nav').classList.toggle('lm-nav-open');
}
...
/* ── IntersectionObserver para animar ao entrar na tela ── */
...
document.querySelectorAll('.animate-fade-up, .lm-stats-section, .lm-step, .lm-cat-card').forEach(el => io.observe(el));